...
from .api.v1.subtasks import router as subtasks_router
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("🚀 Starting InfoRUN API...")
    
    # Check database connection
    if not check_database_connection():
        logger.error("❌ Database connection failed. Application may not work properly.")
    
    logger.info("✅ InfoRUN API started successfully")
    
    yield
    
    # Shutdown
    logger.info("🛑 Shutting down InfoRUN API...")


# Create FastAPI application
app = FastAPI(
    title=settings.app_name,
    description="A modern educational platform API for task management and learning",
    version="1.0.0",
    docs_url="/api/docs",
    redoc_url="/api/redoc",
